class PlayerInteraction {
    ...
    tableroMinas = new Tablero(10,10,5); //Aqui se guardara el tablero
    ...
    clickCasilla() { //Funcion que se ejecuta al clickar una casilla
        let j = this.getAttribute('data-x');
        let i = this.getAttribute('data-y');
         //Se le asigna un evento a cada celda
            if (tableroMinas.tablero[i][j].esCasillaInicial && casilla.classList.contains('bandera') === false) {
                //Codigo que solo se ejecuta la primera vez que se hace click en el tablero
                tableroMinas.tablero[i][j].initialClick = true;
                tableroMinas.colocarBombas(tableroMinas.minas, i, j);
                tableroMinas.colocarNumeros();

                for (let i = 0; i < tableroMinas.altura; i++) {
                    for (let j = 0; j < tableroMinas.anchura; j++) {
                        if (tableroMinas.tablero[i][j].hasBomba === false && tableroMinas.tablero[i][j].numero > 0 ) {
                            let casilla = container.children[i].children[j];
                            casilla.textContent = tableroMinas.tablero[i][j].numero;
                            //Pone el numero a las casillas que no sean 0 y que no contengan una bomba.
                        }
                        if (tableroMinas.tablero[i][j].isDesvelada) {
                            let casilla = container.children[i].children[j];
            ...
            if (tableroMinas.tablero[i][j].hasBomba && casilla.classList.contains('bandera') === false && tableroMinas.tablero[i][j].isDesvelada === false) {
                desvelarTablero(container);
            }
            //Desvela la casilla, si es 0, desvela las casillas adyacentes
            else if (casilla.classList.contains('bandera') === false && tableroMinas.tablero[i][j].isDesvelada === false) {
                tableroMinas.tablero[i][j].isDesvelada = true;
                if (tableroMinas.tablero[i][j].getN() === 0 && casilla.classList.contains('hidden')) {
                    desvelarCerosConectados(container, tableroMinas.tablero, tableroMinas.tablero[i][j].posicionX, tableroMinas.tablero[i][j].posicionY);
                }
                else {
                ...
                if (Tablero.casillasDesveladas >= (tableroMinas.altura * tableroMinas.anchura) - tableroMinas.minas) {
